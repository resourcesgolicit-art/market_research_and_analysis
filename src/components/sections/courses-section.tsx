...
export function CoursesSection() {
  const { courses, courseFilter, setCourseFilter, updateCourseProgress } =
    useDashboard();
      ...
      <CardContent>
        <div className='flex gap-2 mb-4 pb-4 border-b border-border'>
          {(['all', 'in-progress', 'completed'] as const).map((filter) => (
            <Button
              ...
              className={
                courseFilter === filter ? 'bg-secondary text-white' : ''
              }
            >
              {filter === 'all'
                ? 'All'
        ...
        <div className='space-y-4'>
          {filteredCourses.map((course) => (
            <div
              key={course.id}
              className='p-4 rounded-lg border border-border color-black bg-background/50 hover:bg-background transition-colors'
            >
              ...
              {course.status === 'In Progress' && (
                <Button
                  ...
                  onClick={() =>
                    updateCourseProgress(course.id, course.progress + 10)
                  }
                >
                  Continue Learning →
                </Button>
